# HyperFrames AGENTS 样例

> 来源：用户提供的 HyperFrames 开源项目 agents 片段。
> 用途：作为 ClipFlow 后续设计项目级 `AGENTS.md` 和 Skill 路由表的参考样例。
...
- 先设一个总入口 Skill，再从总入口路由到具体工作流。
- 每个 Skill 写清楚输入、输出、适用范围和非目标。
- 把构建、格式化、验证命令写进 `AGENTS.md`，减少 AI 自行猜测。
- 对视频产物必须有专门验证步骤，不能只看代码是否写完。
- 保持渲染确定性，避免时间、随机数、网络请求影响重复生成。
